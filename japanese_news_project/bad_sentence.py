...
def bad(text):
    ...
    # 중복글자 제거(4개 이상)
    f_word = re.sub(r'.*([あ-んア-ン])\1{3,}.*', '', special_symbols)
    # 중복글자 제거(3개 이상)
    t_word = re.sub(r'.*([あうえおけがぎぐげごさしすせそざじずぜぞちつだぢづどなにぬねはひふへばびぶべぼぱぴぷぺぽみむめゆよらるれろわを])\1{2,}.*', '', f_word)
    #아아, 아 등 의미 없는 접속사
    aa_sub=re.sub('あ、|ああ、|ー{2,}', '', t_word)
    #발음을 길게 끄는 표시
